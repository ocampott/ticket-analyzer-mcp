...
- Pi: install the published Pi-managed package with `pi install -l npm:ticket-analyzer-mcp@2.3.0`; update with `pi update npm:ticket-analyzer-mcp`, then restart or reload Pi. Pi is managed by Pi, not the npm global CLI. Pi packages run with full system access, so review the package source.
