...
For a central version pin, install an exact version such as `npm install --global ticket-analyzer-mcp@3.0.0`. The alternative `npm install ticket-analyzer-mcp@3.0.0` is isolated to one project and is not the recommended central policy.
...
ticket-analyzer-mcp setup --dry-run --providers trello,jira --clients pi
```
...
```bash
pi install -l npm:ticket-analyzer-mcp@3.0.0
```
